...
//    private int mTypeFace;
     ...
     * 验证码的背景色
     */
    private int mVerificationCodeBackground = Color.WHITE;
...
//                    2 + mTextRect.height() / 2, mPaint);
            ...
            int count = mVerificationText.length();
            for(int i=0;i<count;i++){
                String s = String.valueOf(mVerificationText.charAt(i));
                mPaint.setTextSize(sp2px(60+getRandom()));
                mPaint.setColor(Color.parseColor(getRandomColor()));
                canvas.drawText(s, getWidth() *(i+1)/ 5 , getHeight()-(getRandom()*2), mPaint);
            }
        }
